            ...
            <form id="step-form" class="space-y-5">
                ...
                @else
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        @if(auth()->user()->profile && auth()->user()->profile->category == 'single_female')
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Boob Size
                            </label>
                        ...
                        @elseif(auth()->user()->profile && auth()->user()->profile->category == 'single_male')
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                Dick Size
                            </label>
...
document.getElementById('step-form').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('{{ route('onboarding.step6.store') }}', {
            method: 'POST',
            body: formData,
            headers: {
